...
func subscribeChunk(chunk []UpsertContactPayload, authKey, listID string) {
	...
	for _, contact := range chunk {
		...
		go func(c UpsertContactPayload) {
			...
			if err != nil {
				slog.Error("Error processing Contact", slog.Any("error", err), slog.Any("contact", c))
			}
		}(contact)
	}

	wg.Wait()
	slog.Info("Processed chunk of contacts", slog.Int("amount", len(chunk)))
}
...
func deleteChunk(chunk []Contact, authKey, listID string) {
	...
	for _, c := range chunk {
		...
		go func(contact Contact) {
			...
			if err != nil {
				slog.Info("Error deleting contact", slog.Any("error", err), slog.Any("contact", contact))
			}
		}(c)
	}

	wg.Wait()
	slog.Info("Deleted chunk of %d contacts", slog.Int("amount", len(chunk)))
}
...
func LoadMailInfo(path string) ([]MailInfo, error) {
	// Open The Config and Get the Things
	f, err := os.Open(path)
	if err != nil {
		return nil, fmt.Errorf("unable to get mail info: %v", err)
	}
	...
	if err != nil {
		return arr, fmt.Errorf("unable to get mail info: %v", err)
	}
...
func GetApiKey(path string) (string, error) {
	...
	if err != nil {
		return auth.Key, fmt.Errorf("unable to get API key: %v", err)
	}
